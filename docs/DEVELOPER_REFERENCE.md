...
| `/search`                  | wired-minimum    | Real query/mutation wiring and validation are in place, but route state treatment still mixes custom blocks instead of shared state primitives.                                   |
...
| `/settings/profile`        | wired-minimum    | Query/mutation wiring and validation are present, but profile route feedback still relies on bespoke state blocks rather than shared loading/empty/error/rate-limited primitives. |
