...
| `/search`                  | production-ready | Search workbench now uses shared loading/empty/error/rate-limited state primitives alongside shared form primitives, with route-level regression coverage for validation and state UX.       |
...
| `/settings/profile`        | production-ready | Profile settings now use shared loading/empty/error/rate-limited state primitives and shared base form primitives, with route-level regression coverage for loading/error/empty/save states. |
